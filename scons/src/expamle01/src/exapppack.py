...
class exapppack:
    ...
    def __procPackInfo(self):
        try:
            ...
            self.packInfo.write(pack('cccc', 'C', 'Y', 'P', 'K'))
    ...
    def __copyItemIntoPack(self, itemName):
        try:
            ...
            count = 0
            for ch in self.filetype[itemName]:
                self.packInfo.write(ch)
                count += 1
                if count >= 64:
                    break
            else:
                assert(count < 64), 'unexpect case'
                self.packInfo.write((64-count)*'\0')
    ...
    def __procPackItems(self):
        self.packOffset = 64 + (32 + 64)*len(self.files)
        try:
