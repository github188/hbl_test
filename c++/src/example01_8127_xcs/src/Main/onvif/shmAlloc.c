 ...
 */
static void __INLINE__ __shm_list_add(uint8_t *memStart, shm_list_t *pnew, shm_list_t *prev, shm_list_t *next)
{
    next->prev = OFFSET_POINTER(pnew, memStart);
    pnew->next = OFFSET_POINTER(next, memStart);
    pnew->prev = OFFSET_POINTER(prev, memStart);
    prev->next = OFFSET_POINTER(pnew, memStart);
}
 ...
 */
static __INLINE__ void shm_list_add(uint8_t *memStart, shm_list_t *pnew, shm_list_t *head)
{
    __shm_list_add(memStart, pnew, head, OFFSET_NODE(memStart, head->next));
}
 ...
 */
static __INLINE__ void shm_list_add_tail(uint8_t *memStart, shm_list_t *pnew, shm_list_t *head)
{
    __shm_list_add(memStart, pnew, OFFSET_NODE(memStart, head->prev), head);
}
...
shm_alloc_handle init_shm_allocator(uint8_t *memStart, uint32_t chunkSize)
{
    ...
    {
        shell_print("paramter is invalid \n");
...
int shm_all_recycle(shm_alloc_handle handle)
{
    shm_alloc_t *info = (shm_alloc_t *)handle;
    free_shm_pool_t *node = NULL;
    ...
    node = (free_shm_pool_t *)(((uint8_t*)info) + sizeof(shm_alloc_t));

    // INIT_SHM_LIST_HEAD
    info->freeParts.prev = OFFSET_POINTER(&(info->freeParts), ((uint8_t*)info));
    info->freeParts.next = OFFSET_POINTER(&(info->freeParts), ((uint8_t*)info));

    node->size = POOLSIZE_TO_EFFTSIZE(info->chunkSize); // for size information
    info->candiate = OFFSET_POINTER(node, ((uint8_t*)info));
...
//                list_debug("cur->size left grow to %d\n", node->size);
            ...
            {   // 内存泄漏
                list_debug("---memory leak!  chunkEnd:%p cur:%p %p %d\n", chunkEnd, cur,node,node->size);
            }
        }
    }
...
void * shm_malloc(shm_alloc_handle handle, size_t size)
{
    ...
    {
        shell_print("shm_malloc parameter invalid %p  %d %x  %d \n ", info,info->chunkSize , info->magicFlag,size );
        return NULL;
    }
...
int shm_add_reference(shm_alloc_handle handle, void *mem)
{
    ...
    {
        list_debug("@@@memory leak!  chunkEnd:%d size:%d\n", chunkEnd, size);
        return -1;
    }
...
void shm_free(shm_alloc_handle handle, void *mem)
{
    ...
    if (size > info->chunkSize)
    {
        list_debug("###memory leak!  size:%d info->chunkSize:%d\n", size,  info->chunkSize);
        return;
    }
...
uint32_t shm_offset(shm_alloc_handle handle, void *mem)
{
    shm_alloc_t *info = (shm_alloc_t *)handle;
...
void *shm_pointer(shm_alloc_handle handle, uint32_t offset)
{
    shm_alloc_t *info = (shm_alloc_t *)handle;
    ...
    return (void *)( (char *)handle + offset );
}
...
char * shm_strdup(shm_alloc_handle handle, const char *s)
{
    ...
    if (len)
        mem = (char *)shm_malloc(info, len + 1);
