/******************************************************************************
 * vehicleCyldReport.cpp:
 * 
 * Copyright 2009 - 2100 HANGZHOU PEGVISION Technology Co.,Ltd.
 * 
 * DESCRIPTION:  主要用于对超远雷电雷达协议进行解析，结果为DSP可用于抓拍的通道号、
 * 车道号、步骤
 *     
 ...
 * --------------------
 ******************************************************************************/
int cyldPPRealLaneNumGet(unsigned char ucVHLaneId)
{
    unsigned int i =0;
    ...
    DSP_INFO("no find ok cyld vehilce lane id\n");
    return -1;
}


/******************************************************************************
 * cyldPPSnapSeqQuery() :根据车检器信息，进行出卡口抓拍
 * 
 * DESCRIPTION:  
 *  
 * Input: 
...
int cyldPPSnapSeqQuery(unsigned int iComId, unsigned char ucCarChannel, unsigned char ucCarSpeed, unsigned char ucCarDir)
{
    ...
    && (g_PPLaneSwitchInfo[iTmpChannel].cGroudCoilDecType == CS_RADAR_SPEED) )
    {
        ...
        DSP_ERROR("cyldPPSnapSeqQuery uiCarInfo:0x%x\n", uiCarInfo);
    	...
    	{
    	    DSP_INFO("cyld radar pp Snap ok,carInfo:0x%x!\n", uiCarInfo);
    	}
    }
    else
    {
        DSP_ERROR("cyldPPSnapSeqQuery coil type:%d coil dec type:%d is error!\n", \
