 ...
 * --------------------
 ******************************************************************************/
int wkPPRealLaneNumGet(unsigned char ucVHLaneId)
{
    unsigned int i =0;
    ...
    DSP_INFO("no find ok wk vehilce lane id\n");
    return -1;
}


/******************************************************************************
 * wkPPSnapSeqQuery() :根据车检器信息，进行出卡口抓拍
 * 
 * DESCRIPTION:  
 *  
 * Input: 
...
int wkPPSnapSeqQuery(unsigned int iComId, unsigned char ucCarChannel, unsigned char ucCarSpeed, unsigned char ucCarDir)
{
    ...
    iTmpChannel = wkPPRealLaneNumGet(ucCarChannel);
    ucCorrectSpeed = ucCarSpeed;
    DSP_INFO("wk radar normal speed:%d, correct speed:%d!\n", ucCarSpeed, ucCorrectSpeed);
    ...
    && (g_PPLaneSwitchInfo[iTmpChannel].cGroudCoilDecType == CS_RADAR_SPEED) )
    {
        ...
        DSP_INFO("wkPPSnapSeqQuery uiCarInfo:0x%x\n", uiCarInfo);
    	...
    	{
    	    DSP_INFO("wk radar pp Snap ok,carInfo:0x%x!\n", uiCarInfo);
    	}
    }
    else
    {
        DSP_ERROR("wkPPSnapSeqQuery coil type:%d coil dec type:%d is error!\n", \
