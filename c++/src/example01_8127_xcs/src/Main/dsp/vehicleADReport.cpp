 ...
 * 
 * DESCRIPTION:  主要用于对安道雷达协议进行解析，结果为DSP可用于抓拍的通道号、
 * 车道号、步骤
 *     
 ...
 * --------------------
 ******************************************************************************/
int adPPRealLaneNumGet(unsigned char ucVHLaneId)
{
    unsigned int i =0;
...
/******************************************************************************
 * adPPSnapSeqQuery() :根据车检器信息，进行出卡口抓拍
 * 
 * DESCRIPTION:  
 *  
 * Input: 
...
int adPPSnapSeqQuery(unsigned int iComId, unsigned char ucCarChannel, unsigned char ucCarSpeed, unsigned char ucCarDir)
{
    ...
    && (g_PPLaneSwitchInfo[iTmpChannel].cGroudCoilDecType == CS_RADAR_SPEED) )
    {
        ...
        DSP_ERROR("csPPSnapSeqQuery uiCarInfo:0x%x\n", uiCarInfo);
