 ...
 * 
 * DESCRIPTION:  主要用于对合肥文康雷达协议进行解析，结果为DSP可用于抓拍的通道号、
 * 车道号、步骤
 *     
 * modification history
 * --------------------
 * 01a, 2012.09.03, wangbin Create
 * --------------------
 ******************************************************************************/
#include <stdio.h>
#include <string.h>
 ...
 * modification history
 * --------------------
 *01a, 2013.10.10, its Create
 * --------------------
 ******************************************************************************/
int olviaPPRealLaneNumGet(unsigned char ucVHLaneId)
{
    unsigned int i =0;
    for(i =0; i< g_uiClientLaneSum; i++)
    {
        if( ucVHLaneId == g_aPassPortChanInfo[i].cModuleId )
        {
            return i;
        }
    }
    DSP_INFO("no find ok olvia vehilce lane id\n");
    return -1;
}


/******************************************************************************
 * olviaPPSnapSeqQuery() :根据车检器信息，进行出卡口抓拍
 * 
 * DESCRIPTION:  
 *  
 * Input: 
...
int olviaPPSnapSeqQuery(unsigned int iComId, unsigned char ucCarChannel, unsigned char ucCarSpeed, unsigned char ucCarDir)
{
    ...
    if( ucCarSpeed == 0 )
    {
        /* 由于奥利维亚雷达在空闲时查询到的速度为0，所以当速度为0时不进行处理 */
        return 0;
    }
    if( ucCarChannel <= 0 )
    {
    ...
    iTmpChannel = olviaPPRealLaneNumGet(ucCarChannel);
    ...
    && (g_PPLaneSwitchInfo[iTmpChannel].cGroudCoilDecType == CS_RADAR_SPEED) )
    {
        ...
        ret = dctlPPFlashNormalSnap(iComId, iTmpChannel, 2, uiCarInfo, ucCarDir);
    }
    ...
    && (g_PPLaneSwitchInfo[iTmpChannel].cGroudCoilDecType == CS_RADAR_SPEED) )
    {
        ...
        DSP_INFO("olviaPPSnapSeqQuery uiCarInfo:0x%x\n", uiCarInfo);
    	...
    	{
    	    DSP_INFO("olvia radar pp Snap ok,carInfo:0x%x!\n", uiCarInfo);
    	}
    }
    else
    {
    	DSP_ERROR("olviaPPSnapSeqQuery car lane:%d,tmpChannel:%d!\n", \
        ucCarChannel, iTmpChannel);
        
        DSP_ERROR("olviaPPSnapSeqQuery coil type:%d coil dec type:%d is error!\n", \
