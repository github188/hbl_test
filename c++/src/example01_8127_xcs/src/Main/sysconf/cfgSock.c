...
#define CONF_LOCAL_NAME         "/tmp/cfgClient"
#define CONF_SLICE_SIZE         (1024)
...
static int cfgSockSend(const char *command)
{
	...
	if (ret != len)
	{
		...
		si_init_flag = 0;
		syslog(LOG_ERR|LOG_USER , "send conf message failed , ret = %d , errno %d\n" , ret , errno)  ;		
	}
...
const char *cfgSockGetValue(const char *a_pSection, const char *a_pKey, const char *a_pDefault)
{
    ...
    nRet = cfgSockRequest(conf_buf, conf_buf, CONF_SLICE_SIZE);
    if (nRet < 0 )
    {
...
//    if (!si_init_flag || !a_pSection || !a_pKey || !a_pValue)
    ...
    if (a_pSection && a_pKey)
    {
        snprintf(conf_buf, CONF_SLICE_SIZE, "D %s.%s %s", a_pSection, a_pKey, a_pValue);
    }
    else
    {
        const char *key = (a_pSection)? a_pSection : a_pKey;

        snprintf(conf_buf, CONF_SLICE_SIZE, "D %s %s", key, a_pValue);
    }
